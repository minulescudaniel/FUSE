...
static int do_chmod(const char* path, mode_t mode) {
    ...
    // Update the file's mode
    struct stat st;
    if (do_getattr(path, &st) != 0) {
        return -errno; // Error getting file attributes
    }

    st.st_mode = (st.st_mode & ~07777) | (mode & 07777);

    if (utimensat(AT_FDCWD, path, NULL, 0) == -1) {
        return -errno; // Error updating the file's timestamps
    }

    if (chmod(path, mode) == -1) {
        return -errno; // Error updating file permissions
    }
...
int main(int argc, char* argv[]) {
    ...
    // Append the FUSE mount point to the current path
    char mount_point[] = "/home/alex/Desktop/PSO/MountPoint"; // Update this to match your FUSE mount point
    strcat(current_path, mount_point);
