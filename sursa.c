...
{
    persistency_directory=(char*)malloc(1024);
    strcpy(persistency_directory,"/home/");
    strcat(persistency_directory,getlogin());
    strcat(persistency_directory,"/Desktop/Memory");
    struct stat st;
    if(stat(persistency_directory,&st))
    {
        mkdir(persistency_directory,0777);
    }
    strcat(persistency_directory,"/");
...
static int do_symlink(const char *target, const char *link_path)
{
    ...
    if (!strchr(link_path, '/'))
    {
        ...
        if (dir)
        {
            create_file(dir, link_path, __S_IFLNK | 0644); 
